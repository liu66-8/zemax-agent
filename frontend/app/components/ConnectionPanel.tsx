...
import { useConnectionStore, useSettingsStore, type AppSettings, type ConnStatus } from "@/app/stores";
import { Wifi, WifiOff, Settings, Loader2, Zap, Database } from "lucide-react";
import { useModal } from "@/app/components/Modal";

export default function ConnectionPanel() {
  const { zos, qdrant, python, connect, disconnect } = useConnectionStore();
    ...
    <header className="topbar">
      ...
      <div className="topbar-status">
        <ConnBadge icon={Zap} name="OpticStudio" state={zos}
          onConnect={() => connect("zos")} onDisconnect={() => disconnect("zos")} />
        <ConnBadge icon={Database} name="Qdrant" state={qdrant}
          onConnect={() => connect("qdrant")} onDisconnect={() => disconnect("qdrant")} />
...
function ConnBadge({ icon: Icon, name, state, onConnect, onDisconnect }: {
  icon: any; name: string; state: { status: ConnStatus; message: string; latencyMs: number };
  ...
  return (
    <span style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 11.5, color: "var(--text-secondary)" }}>
      <div className={`status-dot ${isOn ? "on" : "off"}`}
        style={{ boxShadow: isConnecting ? "0 0 6px var(--warning)" : undefined }} />
      <span>
