...
export default function FileManager() {
  const { currentFile, recentFiles, setCurrentFile, addRecent } = useFileStore();
  ...
  const handleOpenFile = async () => {
    ...
    if (data && data.surfaces) {
      const name = "Loaded_Design.zmx";
      setLensData(data.surfaces as any[], data.effective_focal_length || 0, data.f_number || 0, data.total_track || 0);
      setCurrentFile(name);
      addRecent({ name, path: `workspace/${project?.id || "default"}/${name}`, openedAt: new Date().toISOString() });
    }
    close();
  };
  ...
  const openLoadDialog = () => open(
    <div>
      ...
      <div style={{
        ...
        <p style={{ fontSize: 11, color: "var(--text-tertiary)", marginTop: 4 }}>或通过 OpticStudio 加载</p>
      </div>
      ...
      <div className="modal-actions">
        ...
        <button className="btn btn-primary" onClick={handleOpenFile}>
          <FolderOpen size={14} /> 从 OpticStudio 加载
        </button>
      </div>
    </div>
  );
        ...
        {currentFile ? (
          <div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 6 }}>
              <MiniCard icon={HardDrive} label="数据来源" value="OpticStudio" />
              <MiniCard icon={Clock} label="加载状态" value="已同步" />
              <MiniCard icon={Download} label="关联分析" value="—" />
            </div>
          </div>
        ) : (
