  ...
  return (
    <div className="space-y-4" style={{ padding: "4px 0" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
        <h2 style={{ fontSize: 16, margin: 0 }}>Projects</h2>
        <button onClick={() => setShowingCreate(!showingCreate)} style={btnStyle}>
          {showingCreate ? "Cancel" : "+ New"}
        </button>
      </div>
      ...
      {showingCreate && (
        <div style={{ background: "var(--bg-tertiary)", padding: 12, borderRadius: 6, marginBottom: 12 }}>
          ...
          <button onClick={create} style={{ ...btnStyle, marginTop: 8, width: "100%" }}>Create</button>
        </div>
      )}

      {projects.length === 0 && <p style={{ color: "var(--text-secondary)", fontSize: 13 }}>No projects yet. Create one to get started.</p>}
      ...
      {projects.map((p: any) => (
        <div key={p.id}
          ...
          <div style={{ fontSize: 11, color: "var(--text-secondary)", marginTop: 2 }}>
            {p.system_type || "New"} · {p.design_phase}
          </div>
        </div>
      ))}
    </div>
